...
//Save Skin
new String:g_sSkinModel[MAXPLAYERS+1][128];
...
new String:Tornado[]="ambient/wind/css15_wind_10.wav"; //HL1/ambience/des_wind2.wav
new m_vecBaseVelocity; //offsets
new Float:CycloneVec[9]={0.0,380.0,390.0,400.0,410.0,420.0,430.0,440.0,450.0};
new FaeriedBy[66];

//skill 3
new String:crow[]="ambient/animal/crow.wav"; //"npc/crow/alert3.wav"
...
public OnPluginStart()
{
    ...
    HookEvent("hostage_follows",OnHostageFollow);
    HookEvent("hostage_rescued",OnHostageRescued);
...
public OnRaceChanged(client, oldrace, newrace)
{
    ...
    {
        War3_WeaponRestrictTo(client,thisRaceID,"weapon_knife;weapon_c4");
    ...
    for(new i=1;i<=MaxClients;i++){
        if(ValidPlayer(i,true))
        {
            ...
            if(bFaerie[i]==true)
            {
                ...
                {
                    this_pos[2]+=20;
                    TE_SetupGlowSprite(this_pos,GlowSprite2,0.1,0.6,80);
                    TE_SendToAll();    
                }
            }
        }
...
public OnMapStart()
{
    ...
    GlowSprite2=PrecacheModel("materials/war3source/effects/blueflare.vmt");
    ShieldSprite=PrecacheModel("materials/war3source/sprites/strider_blackball.vmt");
    TornadoSprite=War3_PrecacheBeamSprite();

    PrecacheSoundAny(Tornado);
    PrecacheSoundAny(crow);

//    GlowSprite2=PrecacheModel("VGUI/gfx/VGUI/gign.vmt");
//sprites/blueglow1.vmt
    War3_AddFile("materials/war3source/effects/blueflare.vmt");
    War3_AddFile("materials/war3source/effects/blueflare.vtf");
    War3_AddFile("materials/war3source/sprites/strider_blackball.vmt");
    War3_AddFile("materials/war3source/sprites/strider_blackball.vtf");
}
        ...
        if(vteam!=ateam)
        {
            ...
            if(race_attacker==thisRaceID && skill_level_cyclone>0)
            {
                if(GetRandomFloat(0.0,1.0)<=0.5 && !W3HasImmunity(victim,Immunity_Skills)&&!Silenced(attacker))
                {
                    ...
                    TE_SendToAll();
                    EmitSoundToAllAny(Tornado,attacker);
...
public Action:returnform(Handle:h, any:client)
{
    if(ValidPlayer(client,true))
    {
        ...
        War3_WeaponRestrictTo(client,thisRaceID,"weapon_knife;weapon_c4");
        GivePlayerItem(client, "weapon_knife");
        SetEntityModel(client, g_sSkinModel[client]);
    }
}
...
public OnWar3EventDeath(victim,attacker)
{
    ...
    if(bRescue[victim]==true)
    {
        bRescue[victim]=false;
    }
}

public OnHostageFollow(Handle:event,const String:name[],bool:dontBroadcast)
{
    new client = GetClientOfUserId(GetEventInt(event, "userid"));
    new rescuerrace=War3_GetRace(client);
    if(rescuerrace==thisRaceID)
    {
        bRescue[client]=true;
        if(bFlying[client])
		{
...
public OnHostageRescued(Handle:event,const String:name[],bool:dontBroadcast)
{
    new client = GetClientOfUserId(GetEventInt(event, "userid"));
    new rescuerrace=War3_GetRace(client);
    if(rescuerrace==thisRaceID)
    {
        bRescue[client]=false;
    }
}
