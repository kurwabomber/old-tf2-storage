...
* Updated for 2.4.0 and modified slightly (08/11/2012): Remy Lebeau
*/
...
new HealthArr[]={0,-20,-40,-60,-80,-99};
...
public OnWar3PluginReady()
{
        ...
        SKILL_SUICIDE=War3_AddRaceSkill(thisRaceID,"Detonate","Explode and do damage - autoactivates on death.",true,4);
...
/* ***************************************************************************************************
*
*                PUNISHMENT FOR NAUGHTY - GUN DROPPING - WISPS.
*
*
*****************************************************************************************************/
...
public Action:CS_OnCSWeaponDrop(client, weaponIndex)
{
    ...
    if(ValidPlayer(client,true,true) && War3_GetRace(client)==thisRaceID  &&  StrEqual(wepName, "weapon_usp", false) )
    {
        CreateTimer(2.0,wispWepCheck,client);
    }
}


public Action:wispWepCheck(Handle:timer,any:client)
{
        new String:NameWeapon[64];
        new String:NameNaughty[64];
        GetClientWeapon(client, NameWeapon, 64);
        GetClientName( client, NameNaughty, 64 );
        if (!StrEqual(NameWeapon, "weapon_usp", false))
        {
            PrintToChatAll("\x4%s Has been a naughty wisp!  They shall no longer be hidden.", NameNaughty);
