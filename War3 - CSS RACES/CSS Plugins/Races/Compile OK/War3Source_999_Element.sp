...
public OnPluginStart()
{
	...
	HookEvent("round_end",RoundStartEvent);
}
...
public PlayerDeathEvent( Handle:event, const String:name[], bool:dontBroadcast )
{
	...
	if( War3_GetRace( client ) == thisRaceID && attacker != client && attacker != 0 )
	{
		...
		if( skill_time > 0 && GetRandomFloat( 0.0, 1.0 ) <= TimeChance[skill_time] && !W3HasImmunity( attacker, Immunity_Skills ))
		{
			bTimed[client]=true;
			RoundOver = false;
...
{
	if (ValidPlayer(client) && !RoundOver)
	{
		if (!IsPlayerAlive( client ))
		{
...
{
	if (ValidPlayer(attacker) && !RoundOver)
	{
		if (!IsPlayerAlive( attacker ))
		{
			War3_SpawnPlayer( attacker );
			
			if(War3_GetGame()==Game_CS && !IsPlayerAlive(attacker)){
				//give weapons CS
				for(new s=0;s<10;s++)
				{
				...
				for(new s=0;s<32;s++)
				{
					SetEntData(attacker,AmmoOffset+(s*4),War3_CachedDeadAmmo(attacker,s),4);
				}
				// give them their weapons
				for(new s=0;s<10;s++)
				{
					new String:wep_check[64];
					War3_CachedDeadWeaponName(attacker,s,wep_check,64);
					if(!StrEqual(wep_check,"") && !StrEqual(wep_check,"",false) && !StrEqual(wep_check,"weapon_c4") && !StrEqual(wep_check,"weapon_knife"))
					{
						new wep_ent=GivePlayerItem(attacker,wep_check);
						if(wep_ent>0)
						{
							//dont reduce ammo
							//SetEntData(wep_ent,Clip1Offset,War3_CachedDeadClip1(attacker,s),4);
						}
					}
				}
				SetEntProp(attacker,Prop_Send,"m_ArmorValue",100); //give full armor
			}
		}
