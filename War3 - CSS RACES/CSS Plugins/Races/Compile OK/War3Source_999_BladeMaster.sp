...
new WindSprite; 
new String:ultsnd[]="war3source/bmaster/swiftbladespin.mp3";
...
public OnWar3PluginReady()
{
		...
		thisRaceID=War3_CreateNewRace("Blademaster [WC3:ROC]","bmaster");
		SKILL_WINDWALK=War3_AddRaceSkill(thisRaceID,"Wind Walk (+ability)", "Go invisible and gain speed",false,8);
		SKILL_MIRRORIMAGE=War3_AddRaceSkill(thisRaceID,"Mirror Image (victim)","Confuse your attacker with illusions",false,8);
		SKILL_CRITICAL=War3_AddRaceSkill(thisRaceID,"Critical Strike (attacker)","15% chance to do double or triple damage",false,8);
		ULT_BLADESTORM=War3_AddRaceSkill(thisRaceID,"Bladestorm","Do massive damage to nearby enemies",true,8); 
		...
		if(skill_level>0)
		{
			if(War3_SkillNotInCooldown(client,thisRaceID,SKILL_WINDWALK,true))
			{
				if(!Silenced(client))
				{
					...
					PrintHintText(client,"Wind Walk: Invisibilty");
					bWinded[client]=true;
...
public Action:RemoveInvis(Handle:t,any:client)
{
	if(ValidPlayer(client,true) && bWinded[client]==true)
	{
		...
		War3_SetBuff(client,bImmunitySkills,thisRaceID,false);
		bWinded[client]=false;
		PrintHintText(client,"Wind Walk: Ended");
...
public Action:UltimateLoop(Handle:timer,any:userid)
{
	...
	if(ValidPlayer(attacker) && UltimateZapsRemaining[attacker]>0&&IsPlayerAlive(attacker))
	{
		...
		if(besttarget>0){
			...
			PrintHintText(besttarget,"Hit by Bladestorm");
		}
