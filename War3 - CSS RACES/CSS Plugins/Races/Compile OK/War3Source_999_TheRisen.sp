...
new Float:UltSpeed[7] = { 0.7, 0.8, 0.9, 0.95, 1.0, 1.1, 1.2 };
new HealthADD[7] = { 100, 1100, 1200, 1350, 1600, 1850, 2100 };
new bool:Infected[64];
new InfectedBy[64];
new bool:bIsRisen[MAXPLAYERSCUSTOM];
...
public OnMapStart()
{
    ...
    //War3_AddCustomModel("models/player/ssg/bloodsucker/slow_bloodsucker.mdl");
    PrecacheModel("models/player/ssg/bloodsucker/slow_bloodsucker.mdl", true);
    AddFileToDownloadsTable("models/player/ssg/bloodsucker/slow_bloodsucker.mdl");
    AddFileToDownloadsTable("models/player/ssg/bloodsucker/slow_bloodsucker.dx80.vtx");
    AddFileToDownloadsTable("models/player/ssg/bloodsucker/slow_bloodsucker.dx90.vtx");
    AddFileToDownloadsTable("models/player/ssg/bloodsucker/slow_bloodsucker.phy");
    AddFileToDownloadsTable("models/player/ssg/bloodsucker/slow_bloodsucker.sw.vtx");
    AddFileToDownloadsTable("models/player/ssg/bloodsucker/slow_bloodsucker.vvd");
    AddFileToDownloadsTable("models/player/ssg/bloodsucker/slow_bloodsucker.xbox.vtx");
    AddFileToDownloadsTable("materials/models/player/ssg/bloodsucker/slow_bloodsucker.vmt");
    AddFileToDownloadsTable("materials/models/player/ssg/bloodsucker/slow_bloodsucker.vtf");
    AddFileToDownloadsTable("materials/models/player/ssg/bloodsucker/slow_bump.vtf");
    AddFileToDownloadsTable("materials/models/player/ssg/bloodsucker/slow_free.vtf");
    AddFileToDownloadsTable("materials/models/player/ssg/bloodsucker/slow_sign.vmt");
}

public OnWar3PluginReady()
{
    thisRaceID = War3_CreateNewRace( "The Risen [SSG-DONATOR]", "therisenn" );
    
    SKILL_HP = War3_AddRaceSkill( thisRaceID, "Infected Flesh", "Vastly increase your HP", false, 6 );    
    SKILL_INFECT = War3_AddRaceSkill( thisRaceID, "Infectious Bite", "Once bitten, your enemy WILL DIE", false, 6 );    
    SKILL_SPAWN = War3_AddRaceSkill( thisRaceID, "Rise from the Grave", "RISE! RISE!", false, 6 );
    ULT_SPEED = War3_AddRaceSkill( thisRaceID, "Ferocious Dash", "Burst of speed!", true, 6 );
    
    W3SkillCooldownOnSpawn( thisRaceID, ULT_SPEED, 5.0, _);
...
public InitPassiveSkills( client )
{
    if( War3_GetRace( client ) == thisRaceID )
    {
        ...
        SetEntityModel(client, "models/player/ssg/bloodsucker/slow_bloodsucker.mdl");
    }
}
    ...
    if( race == thisRaceID && pressed && IsPlayerAlive( client ) && !Silenced( client ) )
    {
        ...
        if( ult_level > 0 )
        {
            if( War3_SkillNotInCooldown( client, thisRaceID, ULT_SPEED, true ) )
            {
                ...
                War3_CooldownMGR( client, UltDelay[ult_level], thisRaceID, ULT_SPEED);
            }
        }
        else
        {
...
{
    if( ValidPlayer( client, false ) && bIsRisen[client])
    {
...
Dissolve(any:client)
{
  ...
  if (ragdoll<0)
  {
    PrintToConsole(client, "[DISSOLVE] Could not get ragdoll for player!");  
    return;
  }
  ...
  Format(dtype, sizeof(dtype), "%d", dissolveType);
