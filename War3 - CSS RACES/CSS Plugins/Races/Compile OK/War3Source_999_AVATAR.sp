...
// Sounds
new String:UltSound[] = "ambient/explosions/explode_7.wav";
...
public OnWar3PluginReady()
{
	...
	SKILL_FIRE = War3_AddRaceSkill( thisRaceID, "Fire", "Chance to burn your enemies.", false );
	SKILL_WATER = War3_AddRaceSkill( thisRaceID, "Water", "Wet your enemies so much they slow down.", false );
	SKILL_EARTH = War3_AddRaceSkill( thisRaceID, "Earth", "Shake them like you just don't care.", false );
	ULT_WIND = War3_AddRaceSkill( thisRaceID, "Wind", "Run like the wind (fast and invisible).", true );
	
	W3SkillCooldownOnSpawn( thisRaceID, ULT_WIND, 15.0, _);
	...
	if( race == thisRaceID && pressed && IsPlayerAlive( client ) && !Silenced( client ) )
	{
		...
		if( ult_level > 0 )
		{
			if( War3_SkillNotInCooldown( client, thisRaceID, ULT_WIND, true ) )
			{
				...
				War3_CooldownMGR( client, ( Duration[ult_level] + 10.0 ), thisRaceID, ULT_WIND);
			}
		}
		else
		{
