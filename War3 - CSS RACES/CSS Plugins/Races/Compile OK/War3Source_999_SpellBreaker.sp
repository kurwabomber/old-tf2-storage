...
new String:fdback_sound[]="war3source/spellbreaker/feedback.wav";
...
public OnMapStart()
{
	...
	War3_AddCustomSound(fdback_sound);
			...
			if(War3_SkillNotInCooldown(attacker,thisRaceID,SKILL_STEAL,true)){
				if(War3_GetRace(victim)!=War3_GetRaceIDByShortname("wisp")){
					if(skill_steal>0){
						...
						War3_CooldownMGR(attacker,StealCD[skill_steal],thisRaceID,SKILL_STEAL);
		...
		if(vteam!=ateam){
			...
			if(race_attacker==thisRaceID){
				...
				if(War3_SkillNotInCooldown(attacker,thisRaceID,SKILL_FEEDBACK,true)&&!W3HasImmunity(victim,Immunity_Skills)){
					War3_CooldownMGR(attacker,FeedbackCD[skill_feedback],thisRaceID,SKILL_FEEDBACK);
	...
	if(race==thisRaceID && pressed && ValidPlayer(client,true)){
		if(!Silenced(client)){
			if(War3_SkillNotInCooldown(client,thisRaceID,ULT_CONTROL,true)){
				...
				if(ult_control>0){
					new target = War3_GetTargetInViewCone(client,ControlRange[ult_control],false,15.0);
