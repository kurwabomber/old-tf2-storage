...
public OnMapStart()
{
    PrecacheSound( "weapons/mortar/mortar_explode1.wav", false );
    PrecacheSound( "weapons/mortar/mortar_explode2.wav", false );
    PrecacheSound( "weapons/mortar/mortar_explode3.wav", false );
    PrecacheSound( "weapons/explode3.wav", false );
    PrecacheSound( "weapons/explode4.wav", false );
...
public OnWar3PluginReady()
{
    ...
    SKILL_DMG1 = War3_AddRaceSkill( thisRaceID, "Fighting Thunder", "Do More Damage With grenades", false, 5 );    
    SKILL_DMG2 = War3_AddRaceSkill( thisRaceID, "Thundrbolt", "Release a bolt of thunder", false, 5 );    
    SKILL_DMG3 = War3_AddRaceSkill( thisRaceID, "Staff", "Do more damage With guns", false, 5 );
    SKILL_SHAKE = War3_AddRaceSkill( thisRaceID, "Thunder God", "Shake enemy", false, 5 );
    SKILL_WARD = War3_AddRaceSkill( thisRaceID, "Raidens Realm", "Destroy anyone who passes through Raidens Realm", false, 5 );
    ...
    if( W3GetDamageIsBullet() && ValidPlayer( victim, true ) && ValidPlayer( attacker, true ) && GetClientTeam( victim ) != GetClientTeam( attacker ) )
    {
        if( War3_GetRace( attacker ) == thisRaceID && !W3HasImmunity(victim,Immunity_Skills))
        {
            ...
            if( !Hexed( attacker, false ) && skill_shake > 0 && GetRandomFloat( 0.0, 1.0 ) <= 0.25 )
            {
                ...
                PrintToChat( victim, "\x05: \x04Thunder God" );
                PrintToChat( attacker, "\x05: \x04Thunder God" );
        ...
        if( War3_GetRace( attacker ) == thisRaceID && !W3HasImmunity(victim,Immunity_Skills))
        {
            ...
            if( ValidPlayer( victim, true ) && skill_dmg1 > 0 )
            {
                if( StrEqual( weapon, "hegrenade" ) )
                {
                    ...
                    PrintToChat( victim, "\x05: \x04CRITICAL GRENADE!" );
                    PrintToChat( attacker, "\x05: \x04CRITICAL GRENADE!" );
                ...
                if( tempZ > BeamZ + WARDBELOW && tempZ < BeamZ + WARDABOVE )
                {
                    ...
                    else
                    {
                        if( LastWardClap[wardindex] < GetGameTime() - 1 )
                        {
                            ...
                            PrintToChat( i, "\x05: \x03You got hit by a serpent ward." );
...
stock CreateExplode( client, target, magnitude, radius )
{
    ...
    if( randsound == 1 )
    {
        expsound = "weapons/explode3.wav";
    }
    else if( randsound == 2 )
    {
        expsound = "weapons/explode4.wav";
    }
    else if( randsound == 3 )
    {
        expsound = "weapons/explode5.wav";
    }
    else if( randsound == 4 )
    {
        expsound = "weapons/mortar/mortar_explode1.wav";
    }
    else if( randsound == 5 )
    {
        expsound = "weapons/mortar/mortar_explode2.wav";
    }
    else if( randsound == 6 )
    {
        expsound = "weapons/mortar/mortar_explode3.wav";
    }
