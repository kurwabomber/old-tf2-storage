...
public OnMapStart()
{
	...
	War3_AddCustomSound(Cast);
	War3_AddCustomSound(Imp1);
	War3_AddCustomSound(Imp2);
	War3_AddCustomSound(Imp3);
	War3_AddCustomSound(State);
	War3_AddCustomSound(Exp);
}
	...
	if(IS_PLAYER(victim)&&IS_PLAYER(attacker)&&victim>0&&attacker>0&&attacker!=victim)
	{
		...
		if(vteam!=ateam)
		{
			...
			if(race_attacker==thisRaceID && skill_level>0 && !Silenced(attacker))
			{
				...
				{
					War3_CooldownMGR(attacker,5.0,thisRaceID,SKILL_FROSTARROW);
				...
				if(!Silenced(client)&&War3_SkillNotInCooldown(client,thisRaceID,SKILL_FORKED,true))
				{
					...
					for(new i=0;i<3;i++){
						...
						if(target>0&&!W3HasImmunity(target,Immunity_Skills))
						{
							...
							War3_CooldownMGR(client,11.0,thisRaceID,SKILL_FORKED);
		...
		{	
			...
			if(skill_level>0)
			{
				if(bMShield[client]==true)
				{
					...
					EmitSoundToAll(Exp,client);
				}
				else
				{
					PrintHintText(client,"Mana Shield: Activated");
					EmitSoundToAll(Cast,client);
			...
			if(target>0&&!W3HasImmunity(target,Immunity_Ultimates))
			{
				if(War3_SkillNotInCooldown(client,thisRaceID,ULT_TORNADO,true)) 
				{
					if(!Silenced(client))
					{
						...
						War3_CooldownMGR(client,Cooldown[ult_level],thisRaceID,ULT_TORNADO);
					}
					else
					{
						PrintHintText(client,"Silenced: Can not cast");
