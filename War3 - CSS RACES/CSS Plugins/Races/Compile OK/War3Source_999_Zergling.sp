...
new Float:ChanceJump[5] = { 0.0, 0.6, 0.7, 0.8, 1.0 };
...
new String:attack_sound[] = "npc/roller/mine/rmine_blades_out2.wav";
...
public OnWar3PluginReady()
{
    ...
    SKILL_LONGJUMP = War3_AddRaceSkill( thisRaceID, "Alien Lunge", "You can lunge forward for a quick attack (random chance each spawn)", false );
...
{
    CreateTimer( 2.5, CalcHexHealWaves, _, TIMER_REPEAT );
    ...
    if( race == thisRaceID && pressed && ValidPlayer( client, true ) )
    {
        ...
        if( ult_level > 0 )        
        {
            if( War3_SkillNotInCooldown( client, thisRaceID, ULT_BURROW, true ) ) 
            {
                ToggleBurrow( client );
                AttachSmoke( client );
                War3_CooldownMGR( client, UltCooldown[ult_level], thisRaceID, ULT_BURROW, _, _ );
            }
        }
        else
        {
