...
public OnPluginStart()
{
    ...
    SFXCvar=CreateConVar("war3_azgalor_hugefx_enable","0","Enable/Disable distracting and revealing sfx");
    ...
    if(IS_PLAYER(victim)&&IS_PLAYER(attacker)&&victim>0&&attacker>0&&attacker!=victim)
    {
        ...
        if(vteam!=ateam)
        {
            ...
            if(race_attacker==thisRaceID )
            {
                ...
                if(GetRandomFloat(0.0,1.0)<=Chance[skill_level] && !W3HasImmunity(victim,Immunity_Skills) && skill_level>0 )
                {
                    new Float:spos[3];
                    new Float:epos[3];
    ...
    for(new i=1;i<=MaxClients;i++)
        {
            if(ValidPlayer(i,true)&&GetClientTeam(i)!=AttackerTeam && !bIsTarget[i])
            {
                PrintHintText (client, "Entered into first check" );
                new Float:Vec[3];
                ...
                if(GetVectorDistance(Destination,Vec)<=200.0)
                {
                    ...
                    PrintToChat(client,"[Notice] Expulsion dealing %i damage",magdmg);
...
public OnUltimateCommand(client,race,bool:pressed)
{
    if(race==thisRaceID && pressed && IsPlayerAlive(client))
    {
        ...
        if(skill>0)
        {
            ...
            {
                if (IsLastAliveOnTeam(client))
                {
                    PrintHintText(client,"Can't use rift as last alive on your team");
                }
                else
                {
                    new Float:startpos[3];
        ...
        if(skill_level>0)
        {
            if(!Silenced(client))
            {
                if(War3_SkillNotInCooldown(client,thisRaceID,SKILL_PIT,true))
                {
                    ...
                    new target = War3_GetTargetInViewCone(client,PitMaxDistance[skill_level],false,7.0);
                    ...
                    {
                        PrintHintText(client,"No Valid Target in %f feet",PitMaxDistance[skill_level]/10.0);
                    }
                }
            }
            else
...
public Action:Timer_Rift(Handle:timer, any:client)
{
    ...
    if(skill>0 && War3_GetRace(client) == thisRaceID)
    {
        ...
        TE_SendToAll();
        new morehealth=THISaddhp[skill];
...
static bool:IsLastAliveOnTeam(any:client)
{
    new clientTeam=GetClientTeam(client);
    new playersAliveSameTeam;
    for(new i=1;i<=MaxClients;i++)
    {
        if(i!=client&&ValidPlayer(i,true)&&GetClientTeam(i)==clientTeam)
        {
