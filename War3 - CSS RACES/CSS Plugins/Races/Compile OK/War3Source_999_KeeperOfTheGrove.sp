...
public OnWar3PluginReady()
{
        ...
        SKILL_THORNS=War3_AddRaceSkill(thisRaceID,"Thorns Aura","Deals damage to those who attack you",false,4);
        SKILL_FON=War3_AddRaceSkill(thisRaceID,"Force of Nature (+ability)","Creats an Ent.(+ability)",false,4);
        SKILL_TRANQUILITY=War3_AddRaceSkill(thisRaceID,"Tranquility (+ability1)","Causes a rain of healing energy to fall down, healing teammates.(+ability1)",false,4);
        ULT_ENTANGLE=War3_AddRaceSkill(thisRaceID,"Entangling Roots","Bind enemies to the ground rendering them immobile",true,4);
        ...
        if(skill_level>0)
        {
            ...
            if(War3_SkillNotInCooldown(client,thisRaceID,ULT_ENTANGLE,true)){
                ...
                for(new x=0;x<64;x++)
                {
                    ...
                    War3_ChatMessage(targetList[x],"You have been entangled");//%s!")//,(War3_GetGame()==Game_TF)?", your weapons are POWERLESS until you are released":"");
                ...
                else
                {
                    if(GetConVarBool(EntangleTypeCvar))
                    {
                        PrintHintText(client,"No targets found in %.1f Feet",distance/10.0);
                    }
                    else
                    {
                        PrintHintText(client,"No targets found in %.1f Feet",distance/10.0);
...
public OnAbilityCommand(client,ability,bool:pressed)
{
        ...
        if(War3_GetRace(client)==thisRaceID && ability==0 && pressed && IsPlayerAlive(client))
        {
            ...
            if(skill_level>0)
            {
                if(CurrentWardCount[client]<WardStartingArr[1])
                {
                    ...
                    if(conf_found)
                    {
                        PrintHintText(client,"You can't Summon an Ent Here...");
                    }
                    else
                    {
                        CreateWard(client);
                        CurrentWardCount[client]++;
                        PrintHintText(client,"You Summoned an Ent...");
                        ...
                        playerpos[2]-=75; //enter your height choice there ^.^
                        TeleportEntity(entindex, playerpos, NULL_VECTOR, NULL_VECTOR);
                    }
                }
                else
                {
                    PrintHintText(client,"Youve already summoned your Ent...");
                }    
            }
        }
    }
    if (ability==1){
        if(War3_GetRace(client)==thisRaceID && ability==1 && pressed && IsPlayerAlive(client)){
            ...
            if(skill_level>0){
                if(healwardtimer[client]>=-1337&&healwardtimer[client]<0){
                    ...
                    TE_SendToAll();
                    healwardtimer[client]=0;
                    }
                else {
                    PrintHintText(client,"Wait next round to use this ability...");
                }
            }
            else {
                PrintHintText(client,"Level Tranquility to use Ability1");
            }
        }
    }
}
...
public Action:CalcHealWards(Handle:timer,any:userid){
    ...
    if (ValidPlayer(client, true))
    {
        ...
        if(HealWardLocation[client][0]==0.0&&HealWardLocation[client][1]==0.0&&HealWardLocation[client][2]==0.0){
            ...
            if(healwardtimer[client]<40&&healwardtimer[client]>=0){
                if(healwardtimer[client]>=-1337&&healwardtimer[client]<0){
                }
                else {
                    healwardtimer[client]++;
                    new ownerteam=GetClientTeam(client);    
                    for (new i=1;i<=MaxClients;i++){
                        if(ValidPlayer(i,true)&& GetClientTeam(i)==ownerteam ){
                            GetClientAbsOrigin(i,VictimPos);
                            if(GetVectorDistance(HealWardLocation[client],VictimPos)<250){
                                if(W3HasImmunity(i,Immunity_Skills)){
                                    PrintHintText(i,"Blocking enemy wards");
                                }
                                else {
                                    new DamageScreen1[4];
                                    if(GetClientTeam(i)==2){
                                        DamageScreen1={255,40,40,50};
                                    }
                                    if(GetClientTeam(i)==3){
                                        DamageScreen1={40,40,250,50};
                                    }
                                    W3FlashScreen(i,DamageScreen1);
                ...
                if(tempZ>BeamZ+WARDBELOW && tempZ < BeamZ+WARDABOVE)
                {
					...
					else
					{
						...
						if(LastThunderClap[i]<GetGameTime()-2){
							...
							TE_SetupBeamRingPoint(postionplayer,150.0,FoNDistance[wardradiusskill],BeamSprite,HaloSprite,0,15,2.0,5.0,0.0,DamageScreen2,10,0);
							TE_SendToAll(); 
						}	
					}
                }
