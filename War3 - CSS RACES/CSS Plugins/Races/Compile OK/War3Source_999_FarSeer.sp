...
// new StarSprite;
//,TSprite,CTSprite,BurnSprite,g_iExplosionModel,g_iSmokeModel;
//new Float:fartimer[6]={0.0,5.0,5.0,5.0,5.0,5.0};
//new faralpha[6]={0,50,75,100,125,150};
...
public OnWar3PluginReady(){
		...
		thisRaceID=War3_CreateNewRace("Far Seer [SSG-ADMIN]","seer");
		...
		QUAKEULTI=War3_AddRaceSkill(thisRaceID,"Earthquake (+ultimate)","",true,5);
		...
		if(skill_level>0){
			if(!Silenced(client)){
				if(War3_SkillNotInCooldown(client,thisRaceID,QUAKEULTI,false)){
					War3_CooldownMGR(client,earthquakecool[skill_level],thisRaceID,QUAKEULTI,_,_ );
