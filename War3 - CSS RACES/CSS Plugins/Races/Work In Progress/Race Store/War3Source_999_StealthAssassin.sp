	...
	for(new i=0;i<MaxClients;i++)
	{
		if(ValidPlayer(i)&&War3_GetRace(i)==thisRaceID)
		{
			...
			if(caninvistime[i]<GetGameTime() && invis_level>0)
			{
				...
				War3_SetBuff(i,bInvisWeaponOverride,thisRaceID,true);
				War3_SetBuff(i,iInvisWeaponOverrideAmount,thisRaceID,0);
			}
			else
			{
				War3_SetBuff(i,fInvisibilitySkill,thisRaceID,1.0);
				War3_SetBuff(i,bInvisWeaponOverride,thisRaceID,false);
			}
...
public OnWar3LoadRaceOrItemOrdered(num)
{
	if(num==49)
	{
		thisRaceID=War3_CreateNewRace("Stealth Assassin[HoN]","stealth");
		SKILL_SMOKE=War3_AddRaceSkill(thisRaceID,"Smoke Screen(ability)","Smoke silences and slows enemies",false,6);
		SKILL_STAB=War3_AddRaceSkill(thisRaceID,"Back Stab(attacker)","Damage Boost if you attack from behind",false,6);
		...
		if (ability==0)
		{
			if(War3_GetRace(client)==thisRaceID && ability==0 && pressed && IsPlayerAlive(client))
			{
				...
				if(smoke_level>0)
				{
					...
					{
						War3_CooldownMGR(client,10.0,thisRaceID,SKILL_SMOKE,_,_);
...
public CooldownUltimate(client)
{
	...
	War3_CooldownMGR(client,ASSASSINATION_cooldown[skilllevel_assassination],thisRaceID,ULT_ASSASSINATION,false,_);
}
