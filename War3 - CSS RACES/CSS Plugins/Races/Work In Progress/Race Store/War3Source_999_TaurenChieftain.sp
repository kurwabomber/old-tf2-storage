...
public OnPluginStart()
{
	...
	RespawnDelayCvar=CreateConVar("war3_orc_respawn_delay","4","How long before spawning for reincarnation?");
}
...
public OnWar3LoadRaceOrItemOrdered(num)
{
	if(num==20)
	{
		thisRaceID=War3_CreateNewRace("Tauren Chieftain [WC3:ROC]","taurenc");
		SKILL_SHOCKWAVE=War3_AddRaceSkill(thisRaceID,"Shock Wave (attacker)","Cause damage in a line",false,8);
		SKILL_WARSTOMP=War3_AddRaceSkill(thisRaceID,"War Stomp (+ability)","Slam the ground, dmage and stun nearby enemies",false,8);
		SKILL_ENDURANCE=War3_AddRaceSkill(thisRaceID,"Endurance (passive)","Increase speed for teamates",false,8);
		ULT_REINCARNATION=War3_AddRaceSkill(thisRaceID,"Reincarnation (death)","Come back to life",true,8); 
		...
		if(skill_level>0)
		{
			if(War3_SkillNotInCooldown(client,thisRaceID,SKILL_WARSTOMP,true))
			{
				if(!Silenced(client))
				{
					...
					PrintHintText(client,"War Stomp");
				}
				else
				{
			...
			if(ValidPlayer(i,true)&&GetClientTeam(i)!=team&&!W3HasImmunity(i,Immunity_Skills))
			{
				...
				if(victimdistance<WarStompRadius&&FloatAbs(otherVec[2]-WarStompOrigin[attacker][2])<50)
				{
					if(FloatAbs(victimdistance-damagingRadius)<(WarStompRadius/10.0))
					{
						...
						PrintHintText(i,"You were hit by War Stomp!");
					}
				}
			}
		}
		...
		if(vteam!=ateam)
		{
			...
			if(race_attacker==thisRaceID && skill_level_shockwave>0 && !Silenced(attacker))
			{
				if(GetRandomFloat(0.0,1.0)<=ShockWaveChance[skill_level_shockwave] && !W3HasImmunity(victim,Immunity_Skills))
				{
					...
					PrintToConsole(attacker,"Shockwave");
		...
		if(race==thisRaceID&&!bReincarnation[index]&&War3_GetGame()!=Game_TF)
		{
			...
			if(skill>0&&!Silenced(index))
			{
				...
				if(GetRandomFloat(0.0,1.0)<=percent)
				{
					...
					PrintHintText(index,"REINCARNATION IN %.1f SECONDS!",delay_spawn);
...
public Action:RespawnPlayer(Handle:timer,any:userid)
{
	...
	if(client>0&&!IsPlayerAlive(client))
	{
		...
		War3_ChatMessage(client,"Reincarnated");
	}
	else{
