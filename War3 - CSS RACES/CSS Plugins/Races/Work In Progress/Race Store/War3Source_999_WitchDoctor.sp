/**
* File: War3Source_WitchDoctor.sp
...
new String:stasis[]="ambient/energy/zap9.wav";
...
{
		thisRaceID=War3_CreateNewRace("WitchDoctor", "wdoctor");
		SKILL_TRAINING=War3_AddRaceSkill(thisRaceID,"Witch Doctor Training", "Become stronger and wiser to use more wards",false,3);
		SKILL_SENTRY=War3_AddRaceSkill(thisRaceID,"Sentry Ward (Ability)","Create a ward that spots invisible enemies",false,6);
		SKILL_STASIS=War3_AddRaceSkill(thisRaceID,"Stasis Trap Ward (Ability1)","Create a trap that stuns your enemy and damages him",false,6);
		ULT_HEALING=War3_AddRaceSkill(thisRaceID,"Healing Ward (Ultimate)","Create a ward that heals your allies",false,6);
...
public OnRaceChanged(client, oldrace, newrace)
{
	...
	if(newrace == thisRaceID){
		SetEntityModel(client, "models/player/slow/amberlyn/re5/voodoo_majini/slow.mdl");
		Passive(client);
...
public Passive(client){
	...
	War3_SetBuff( client, iAdditionalMaxHealthNoHPChange, thisRaceID, War3_GetMaxHP(client)+THIShealth[skill_training]);	
	//War3_SetMaxHP(client,War3_GetMaxHP(client)+THIShealth[skill_training]);
	...
	if(!Silenced(client)){
		...
		if(War3_GetRace(client)==thisRaceID && ability==0 && pressed && IsPlayerAlive(client)){
			if(!bDidCastYet[client]){	
				...
				{
					PrintHintText(client, "Level your Sentry Ward first");
				}
				
			}
			else
			{
				PrintHintText(client, "You've already used your ward");
			}
				...
				if(skill_stasis>0){
					if(skill_training>0){
						if(War3_SkillNotInCooldown(client,thisRaceID,SKILL_STASIS,true)){
							...
							War3_CooldownMGR(client,15.0,thisRaceID,SKILL_STASIS);
							CreateTimer(15.0, disableStasis, client);
						}
					}
					else
					{
						PrintHintText(client, "You need more training");
					}
					
				}
				else
				{
					PrintHintText(client, "Level your Stasis Trap Ward first");
				}
						...
						for (new i=1;i<=MaxClients;i++){
							if(ValidPlayer(i,true)&& GetClientTeam(i)!=ownerteam&&bStasis[client]){
								...
								if(GetVectorDistance(StasisWardLocation[client],victimPos)<StasisRad[skill_stasis]){
									PrintHintText(i, "You've fallen for a trap");
									bStasis[client]=false;
...
public OnUltimateCommand(client,race,bool:pressed)
{
	if(race==thisRaceID && pressed && ValidPlayer(client,true)){
		if(War3_SkillNotInCooldown(client,thisRaceID,ULT_HEALING,true)){
			...
			if(ult_healing>0){
				if(skill_training>2){
					if(War3_SkillNotInCooldown(client,thisRaceID,ULT_HEALING,true)){
						...
						War3_CooldownMGR(client,5.0,thisRaceID,ULT_HEALING);
			...
			{
				PrintHintText(client, "Level your Healing Ward first");
			}
