...
public OnWar3LoadRaceOrItemOrdered(num)
{
        if(num==2)
        {
        thisRaceID=War3_CreateNewRace("Paladin[RoC]","paladin");
        SKILL_DEFENSIVE=War3_AddRaceSkill(thisRaceID,"Defensive Aura","Give your teammates extra armor.",false,4);
        SKILL_OFFENSIVE=War3_AddRaceSkill(thisRaceID,"Offensive Aura","Give your teammates extra damage.",false,4);
        SKILL_HEALING=War3_AddRaceSkill(thisRaceID,"Healing Aura","Give your teammates passive regeneration.",false,4);
        ULT_OVERDRIVE=War3_AddRaceSkill(thisRaceID,"Aura Overdrive","Activate all auras at once.",true,4); 
        ...
        for(new i=1;i<=MaxClients;i++)
        {
                if(ValidPlayer(i,true))
                {
                        ...
                        if(War3_GetRace(i)==thisRaceID)
                        {
                                paladin = i;
                                // Same team?
                                if(GetClientTeam(paladin)==GetClientTeam(client))
                                {
...
public Action:UpdateInfoTimer(Handle:timer) 
{
        for(new i=1;i<=MaxClients;i++) 
        {
                if (ValidPlayer(i,true) && !IsFakeClient(i) && (War3_GetRace(i)==thisRaceID)) 
                {
                        ...
                        ShowSyncHudText(i, HudMessage, "Holy Points %s", message);
                }
        }
}
...
public OnAbilityCommand(client,ability,bool:pressed)
{
        if(War3_GetRace(client)==thisRaceID && ability==0 && pressed && IsPlayerAlive(client))
        {
                ...
                else
                {
                        ...
                        SetMenuTitle(menu, "Wich aura do you want to select?");
                        // Only add those auras that have skillpoints
                        if(skill_defensive>0)
                        {
                                AddMenuItem(menu, "defensive", "Defensive Aura");
                        }
                        if(skill_offensive>0)
                        {
                                AddMenuItem(menu, "offensive", "Offensive Aura");
                        }
                        if(skill_healing>0)
                        {
                                AddMenuItem(menu, "healing", "Healing Aura");
                        }
...
public OnUltimateCommand(client,race,bool:pressed)
{
        if(race==thisRaceID && pressed && IsPlayerAlive(client))
        {
                ...
                {
                        PrintHintText(client, "You went into overdrive!");
                ...
                {
                        PrintHintText(client, "Not enough Holy Points!");
                }
        }
}
...
public Action:ReduceOverdrive(Handle:timer,any:client)
{
        if(overdriveCharge[client]<=10)
        {
                overdriveCharge[client] = 0;
                bOnOverdrive[client] = false;
                PrintHintText(client, "Finished overdrive");
        }
        else
        {
