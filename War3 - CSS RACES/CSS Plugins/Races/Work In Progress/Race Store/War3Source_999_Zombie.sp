...
new Float:MangleChance[]={0.0, 0.2,0.24,0.28,0.32,0.36};
...
new WardStartingArr[]={0,1,1,2,2,3}; 
...
new Float:LurchTime[]={0.0, 8.0, 8.5, 9.0, 9.5, 10.0};
 
new groanhealth[]= {0, 12, 14, 16, 18, 20};
...
new Float:RadArr[]={0.0, 220.0, 240.0, 260.0, 280.0, 300.0};
new ExplosionModel;
...
public OnWar3LoadRaceOrItemOrdered2(num)
{
        if(num==111)
        {
        thisRaceID=War3_CreateNewRace("[Gold]Zombie", "zombie");
        SKILL_MANGLE=War3_AddRaceSkill(thisRaceID,"Mangle(attacker)", "More damage faster infection",false,5);
        SKILL_LURCH=War3_AddRaceSkill(thisRaceID,"Lurch(ability)", "Normal speed",false,5);
        SKILL_RETCH=War3_AddRaceSkill(thisRaceID,"Retch(+ability1)", "Healing Infecting Cloud",false,5);
        SKILL_GROAN=War3_AddRaceSkill(thisRaceID,"Groan(+ability2)", "Call other zombies to your side",false,5);
        ULT_EXPLOSION=War3_AddRaceSkill(thisRaceID,"Explosion", "Focus and explode infecting the enemy",true,5);
...
public RoundEndEvent(Handle:event,const String:name[],bool:dontBroadcast)
{
        for(new i=1;i<=MaxClients;i++)
        {
                if(ValidPlayer(i))
                {
                        if(War3_GetRace(i)!=thisRaceID && bZombied[i])
                        {
...
public OnWar3EventDeath(victim,attacker)
{
        if(ValidPlayer(victim)&&ValidPlayer(attacker))
        {
                ...
                newrace2 = War3_GetRace(victim);
                if(newrace2==thisRaceID)
                {
                        GetClientAbsOrigin(victim,ExplosionLocation[victim]);
...
public OnWar3EventSpawn(client, newrace)
{
...
{
        if(newrace==thisRaceID && pressed && ValidPlayer(client,true))
        {
                ...
                {               
                        ...
                        {
                                War3_CooldownMGR(client,5.0,thisRaceID,ULT_EXPLOSION,_,_);
        ...
        if(!Silenced(client))
        {
                if(War3_GetRace(client)==thisRaceID && ability==0 && pressed && IsPlayerAlive(client))
                {
                        ...
                        if(lurch>0)
                        {
                                ...
                                {
                                        War3_CooldownMGR(client,12.0,thisRaceID,SKILL_LURCH,_,_);
                                        War3_SetBuff(client,fSlow,thisRaceID,1.0);
                ...
                if(War3_GetRace(client)==thisRaceID && ability==2 && pressed && IsPlayerAlive(client))
                {
                        ...
                        if(skill_level>0)
                        {
                                ...
                                {
                                        War3_CooldownMGR(client,10.0,thisRaceID,SKILL_GROAN,_,_);
                                        EmitSoundToAll(groan,client);
