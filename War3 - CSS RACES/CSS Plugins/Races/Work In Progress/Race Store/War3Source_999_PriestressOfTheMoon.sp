...
new Float:BurnTime[5]={0.0,3.0,4.0,5.0,6.0};
...
public OnWar3LoadRaceOrItemOrdered2(num)
{
	if(num==150)
	{
		thisRaceID=War3_CreateNewRace("[BETA]Priestess Of The Moon","potm");
		SKILL_HIDE=War3_AddRaceSkill(thisRaceID,"Hide","Invisibility,: More if crouched.",false,4);
		SKILL_SCOUT=War3_AddRaceSkill(thisRaceID,"Scout (+ability)","Detects all enemy units. (+ability)",false,4);
		SKILL_SEARING=War3_AddRaceSkill(thisRaceID,"Searing Arrows","Your Bullets may burn your enemy",false,4);
		SKILL_TRUESHOT=War3_AddRaceSkill(thisRaceID,"Trueshot Aura","Your attacks deal more damage",false,4);
		ULT_STARFALL=War3_AddRaceSkill(thisRaceID,"Star Fall(+ultimate)","A shooting star falls from the sky and damages nearby enemies",true,4);
...
public OnRaceChanged(client,oldrace,newrace)
{
	...
	if(newrace == thisRaceID){
		War3_WeaponRestrictTo(client,thisRaceID,"weapon_scout");
		...
		if(vteam!=ateam)
		{
			...
			if(race_attacker==thisRaceID && skill_level_trueshot>0 && IsPlayerAlive(attacker))
			{
				...
				{  
					...
					PrintHintText(attacker,"TrueShot!");
					PrintHintText(victim,"Hit by TrueShot!");
				}
			}
				...
				if(ValidPlayer(i,true)&& GetClientTeam(i)!=ownerteam ){
					...
					if(GetVectorDistance(starfallstar[client],VictimPos)<200){
						...
						if(!W3HasImmunity(i,Immunity_Skills)){
							War3_DealDamage(i,70,client,DMG_CRUSH,"Falling Star",_,W3DMGTYPE_MAGIC);
							PrintHintText(i,"Star Fall: 70 damage!");
						}
					}
				}
			}
						...
						if(ValidPlayer(i,true)&&IsPlayerAlive(i)){
							...
							{							
								if(GetClientTeam(i)==3){
									TE_SetupBeamPoints(position, positionclient, BeamSprite,HaloSprite, 0, 8, 0.5, 10.0, 10.0, 10, 1.0, {0,0,255,255}, 70); 
							...
							if(GetClientTeam(client)==3)
							{
								if(GetClientTeam(i)==2){
									TE_SetupBeamPoints(position, positionclient, BeamSprite,HaloSprite, 0, 8, 0.5, 10.0, 10.0, 10, 1.0, {255,0,0,255}, 70); 
									TE_SendToClient(client,0.0);
								}
							}
						}
