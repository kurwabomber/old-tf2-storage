...
new String:ARCANESound[]="weapons/physcannon/energy_sing_flyby2.wav";
new String:snd[]="weapons/explode4.wav";
new String:amr[]="physics/concrete/boulder_impact_hard4.wav";
new FlameSprite;
new Float:ChainDistanced[5]={0.0,150.0,200.0,250.0,300.0};
new spelldmg[5]={0,20,40,50,65};
//new Float:spelldmg2[5]={0.0,10.0,20.0,30.0,45.0};
...
public OnWar3EventSpawn(client){
	...
	{ 
		...
		iVec2[2]+=110.0;
		iVec2[0]+=20.0;
		TE_SetupBeamPoints(iVec2,iVec,BeamSprite,HaloSprite,0,41,3.6,6.0,15.0,0,4.5,{25,25,25,255},45);
		TE_SendToAll();
		iVec2[0]-=40.0;
		TE_SetupBeamPoints(iVec2,iVec,BeamSprite,HaloSprite,0,41,3.6,6.0,15.0,0,4.5,{25,25,25,255},45);
		TE_SendToAll();
...
public DoSpell(client,Float:distance,dmg,bool:first_call,last_target)
{
	...
	if(target<=0)
	{
		if(first_call)
		{
			...
			War3_CooldownMGR(client,3.0,thisRaceID,ULT_ARCANE,_,_);
		}
		else
		{
	...
	else
	{
		...
		TE_SendToAll(0.3);
		TE_SetupBeamRingPoint(start_pos,5.0,distance,FireBall,HaloSprite,2,9,8.0,200.0,7.0,{255,255,255,90},50,0);
		TE_SendToAll(1.2);
		TE_SetupBeamRingPoint(start_pos,5.0,distance,FireBall,HaloSprite,2,9,8.0,200.0,7.0,{255,255,255,90},50,0);
		TE_SendToAll(1.4);
		new race = War3_GetRace(client);
