...
public Action:Timer_GlacialStormFX(Handle:timer, Handle:pack) //some point of lazy-ness :S
{
    ...
    if(bUltActive[client]==true&&ValidPlayer(client,false)) {
        ...
        TE_SetupBubbles_Custom(aimVec,aimVec,radius,Glow,12,GetRandomFloat(28.0,150.0));
        TE_SendToAll();
...
stock TE_SetupBubbles_Custom(Float:startposi[3],Float:finalposi[3],Float:fHeight,nModelIndex,nCount,Float:fSpeed)
{
    TE_Start("Bubbles");
