...
public OnPluginStart()
{
	...
	g_hTeam = CreateConVar("css_anti_team_flash_team", "0", "If enabled, players will be unable to team flash their teammates.", FCVAR_NONE, true, 0.0, true, 1.0);
	...
	if(g_bEnabled)
	{
		...
		else
		{
			...
			if(g_iTeam[client] == _iData[1])
			{
				...
				if(g_bTeam || (g_bOwner && _iData[0] == client))
					SetEntPropFloat(client, Prop_Send, "m_flFlashMaxAlpha", 0.5);
			}
		}
	}
