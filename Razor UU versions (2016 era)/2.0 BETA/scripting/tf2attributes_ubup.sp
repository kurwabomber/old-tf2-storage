...
//Detection against gaining other attributes.
new _:current_primary[MAXPLAYERS + 1]
new _:current_secondary[MAXPLAYERS + 1]
new _:current_melee[MAXPLAYERS + 1]

new _:new_primary[MAXPLAYERS + 1]
new _:new_secondary[MAXPLAYERS + 1]
new _:new_melee[MAXPLAYERS + 1]
...
{
	new client = GetClientOfUserId(GetEventInt(event, "userid"));
	if(IsClientInGame(client) && IsValidClient(client) && GetClientMenu(client) == MenuSource_None){
		FakeClientCommand(client, "menuselect 0");
		TF2Attrib_ClearCache(client);
	}
	GetNewWeapons(client)
	RespawnEffect(client);
...
public OnEntityCreated(entity, const char[] classname)
{
    ...
    {
        if(IsValidEdict(entity)){
			new Float:health = (StartMoney+additionalstartmoney*mvmadditional*1.5);
...
GetCurrentWeapons(client)
{
	current_primary[client] = GetPlayerWeaponSlot(client,0);
	current_secondary[client] = GetPlayerWeaponSlot(client,1);
	current_melee[client] = GetPlayerWeaponSlot(client,2);
}
GetNewWeapons(client)
{
	new_primary[client] = GetPlayerWeaponSlot(client,0);
	new_secondary[client] = GetPlayerWeaponSlot(client,1);
	new_melee[client] = GetPlayerWeaponSlot(client,2);
	
	if(new_primary[client] != current_primary[client] || new_secondary[client] != current_secondary[client] || new_melee[client] != current_melee[client])
	{
		FakeClientCommand(client,"menuselect 0");
	}
}
