			...
			if (slot < 3)
			{
				...
				else if (current_class[client] == _:TFClass_Sniper)
				{
					if (!strcmp(classname, "tf_wearable"))
					{
						...
						currentitem_catidx[client][1] = GetUpgrade_CatList("tf_w_darws")
						}
						else
						{
							currentitem_catidx[client][slot] = GetUpgrade_CatList(classname)
...
stock int TF2_GetPlayerHealth(int client) {
	return GetEntProp(GetPlayerResourceEntity(), Prop_Send, "m_iHealth", _, client);
}
/*
public OnGameFrame()
{
	for (new client = 0; client < MAXPLAYERS + 1; client++)
	{
		...
		PrintToChat(client, "Step 0 Done");
		new Address:regenerate = TF2Attrib_GetByName(client, "mod medic killed minicrit boost");
		if(regenerate != Address_Null)
		{
			PrintToChat(client, "Step 1 Done");
			new HealthRegenerated = RoundToNearest(TF2Attrib_GetValue(regenerate)/50);
			new CurrentHealth = TF2_GetPlayerHealth(client)
			new MaxHealth = TF2_GetPlayerMaxHealth(client)
			if(CurrentHealth < MaxHealth && HealthRegenerated != 0)
			{
				PrintToChat(client, "Step 2 Done");
				if(CurrentHealth + HealthRegenerated > MaxHealth)
				{
					SetEntityHealth(client,MaxHealth);
					PrintToChat(client, "Step 3 Done");
				}
				else
				{
					SetEntityHealth(client, CurrentHealth+HealthRegenerated);
					PrintToChat(client, "Step 3 Done");
				}
			}
		}
...
public Action:OnPlayerRunCmd(client, &buttons, &impulse, Float:vel[3], Float:angles[3], &weapon)
{
	...
	new mvmCash = GetEntProp(client, Prop_Send, "m_nCurrency");
	if(CurrencyOwned[client] >= 300000000000.0 || CurrencyOwned[client] < 0){
	...
	if (IsValidClient(client))
	{
		...
		Format(Buffer, sizeof(Buffer), "%.0f$", CurrencyOwned[client]); 
		SetHudTextParams(0.85, 0.8, 0.015, 224, 50, 15, 255);
		ShowHudText(client, -1, Buffer);
		decl String:Startcash[32]
		Format(Startcash, sizeof(Startcash), "%.0f Startmoney", StartMoney+additionalstartmoney); 
		SetHudTextParams(0.85, 0.7, 0.015, 224, 50, 15, 255);
		ShowHudText(client, -1, Startcash);
		if(mvmCash > 0)
		{
			SetEntProp(client, Prop_Send, "m_nCurrency", 0);
		}
...
public Action:Event_PlayerBuff(Handle:event, const String:name[], bool:dontBroadcast)
{
	new client = GetClientOfUserId(GetEventInt(event, "buff_owner"));
	CreateTimer(0.3, buffDelay, client); 
}
public Action:buffDelay(Handle:timer, any:client) 
{
	...
	if ((!IsMvM()))
	{
		...
		if (IsValidClient(attack, false) && IsValidClient(client) && attack != client)
		{
			...
			{ 
				if (IsClientInGame(i) && IsValidClient(i)) 
				{
					CurrencyOwned[i] += (MoneyBonusKill + ((additionalstartmoney + StartMoney) /400))
					PrintToChat(i, "+%.0f$",  (MoneyBonusKill + ((additionalstartmoney + StartMoney)/400)))
				} 
			}  
		...
		if(!IsValidClient(client) && attack != client)
		{
			...
			{ 
				if (IsClientInGame(i) && IsValidClient(i)) 
				{
					CurrencyOwned[i] += (MoneyBonusKill + ((StartMoney+additionalstartmoney)/1200));
					PrintToChat(i, "+%.0f$", (MoneyBonusKill + ((StartMoney+additionalstartmoney)/1200)));
				} 
			}  
...
public Event_tankdown(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{ 
		if (IsClientInGame(i) && IsValidClient(i)) 
		{
			CurrencyOwned[i] +=	((MoneyBonusKill*mvmadditional*12 + ((additionalstartmoney+StartMoney)/300)));
			PrintToChat(i, "+%.0f$ - Boss Killed", (MoneyBonusKill*mvmadditional*12)+ ((additionalstartmoney+StartMoney)/300))
		} 
	}  
...
public Action:mvm_CheckPointAdjustCash(Handle:timer, any:userid)
{
	...
	{
		CurrencyOwned[client] -= (client_spent_money_mvm_chkp[client][0] + client_spent_money_mvm_chkp[client][1] + client_spent_money_mvm_chkp[client][2] + client_spent_money_mvm_chkp[client][3]);
		client_respawn_checkpoint[client] = 0
		CreateTimer(0.05, WeaponReGiveUpgrades, GetClientUserId(client));
	}
}
...
public UberShopinitMenusHandlers()
{
	...
	HookEvent("deploy_buff_banner", Event_PlayerBuff)
		...
		{//
			if (param2 >= 0)
			{
				...
				if (u != 20000)
				{
					...
					new String:fstr[100]
					Format(fstr, sizeof(fstr),"%.0f$ ***%s - Choose attribute:", CurrencyOwned[client], current_slot_name[s]);
					Menu_TweakUpgrades_slot(client, s);
...
public MenuHandler_BuyNewWeapon(Handle:menu, MenuAction:action, mclient, param2)
{
	if (action == MenuAction_Select)
	{
		if (CurrencyOwned[mclient] >= 7500 && client_spent_money[mclient][3] == 0)//if(GetEntProp(wearable, Prop_Send, "m_iItemDefinitionIndex") == 1157)
		{
...
public OnEntityCreated(entity, const char[] classname)
{
    if(StrEqual(classname, "tank_boss"))
    {
        if(IsValidEdict(entity) && IsValidEntity(entity)){
			new Float:health = (StartMoney+additionalstartmoney*mvmadditional*4.0);
			new hp = RoundToNearest(health);
			new Float:spd = (75+mvmadditional+mvmadditional);
			new speed = RoundToNearest(spd);
			new TankHealth = GetEntProp(entity, Prop_Send, "m_iHealth");
			if(TankHealth + hp < 2100000000 || TankHealth + hp > 0)
			{
				SetVariantInt(hp); 
			...
			if(!IsMvM())
			{
				if(StartMoney+additionalstartmoney > 40000)
				{
					new Float:tankArmor = (40000.0/(StartMoney+additionalstartmoney));
			...
			{
				if((StartMoney+additionalstartmoney)*mvmadditional > 50000)
				{
					new Float:tankArmor = (50000.0/(StartMoney+additionalstartmoney))/mvmadditional;
					new Float:tankRes = (1/tankArmor);
					PrintToChatAll("Tank Armor: %f ", tankRes); 
				}
			}
...
UpgradeBots(client)
{
	...
	if(!IsMvM())
	{
		...
		if (current_class[client] == _:TFClass_Scout)
		{
			...
			TF2Attrib_SetByName(secondary,"fire rate bonus",1.4);//Firerate
			/*
			if((additionalstartmoney+StartMoney) <= 75000){
			TF2Attrib_SetByName(primary,"damage bonus HIDDEN",((additionalstartmoney+StartMoney)/15000.0));//Bullets Per shot
			}else{
			TF2Attrib_SetByName(primary,"damage bonus HIDDEN",(5.0));//Bullets per shot
			}
			*/
