...
public Action:OnTakeDamage(victim, &attacker, &inflictor, &Float:damage, &damagetype, &weapon, Float:damageForce[3], Float:damagePosition[3], damagecustom)
{
	if(victim > 0 && victim <= MaxClients && weapon != -1 && (attacker > 0 || attacker <= MaxClients))
	{
		...
		{
			SplashDamage(victim, fl_SplashDamage[attacker], fl_SplashRadius[attacker], damage, damagetype, attacker, weapon);
		}
		if(attacker != victim){
		...
		GetEdictClassname(inflictor, iAttackerObject, sizeof(iAttackerObject5));
...
SplashDamage(client, Float:dmg, Float:distance, Float:damageoriginal, damagetype, enemy, weapon)
{
	if(IsPlayerAlive(client))
	{
		// Find User Location
		...
		for(new i=1; i<=MaxClients; i++)
		{
			...
			if(GetClientTeam(i) == GetClientTeam(client) && GetVectorDistance(uservec, targetvec, false) < distance)
			{
				...
				GetEntPropVector(i, Prop_Data, "m_vecVelocity", force);
				force[2] = 500.0;
				SDKHooks_TakeDamage(i, enemy, enemy, dmg*damageoriginal, damagetype, weapon, force, NULL_VECTOR);
			}
		}
	}//
}
...
public OnEntityCreated(entity, const char[] classname)
{
	...
	if(StrEqual(classname, "obj_sentrygun"))
    {
		...
		{
			SDKHook(entity, SDKHook_OnTakeDamage, OnTakeDamagePre_Sentry); 
			CreateTimer(5.0, BuildingRegeneration, EntIndexToEntRef(entity), TIMER_REPEAT|TIMER_FLAG_NO_MAPCHANGE);
		}
	}
...
public Action:OnTakeDamagePre_Sapper(victim, &attacker, &inflictor, &Float:damage, &damagetype, &weapon, Float:damageForce[3], Float:damagePosition[3], damagecustom) 
{
	...
	new melee = (GetPlayerWeaponSlot(attacker,2));
	new Address:damagebonus = TF2Attrib_GetByName(melee, "damage penalty");
	if(damagebonus != Address_Null)
	{
		new Float:dmgbonus = TF2Attrib_GetValue(damagebonus);
		damage *= dmgbonus;
	}
	new Address:armorpiercing = TF2Attrib_GetByName(melee, "dmg penalty vs players");
	if(armorpiercing != Address_Null)
	{
		new Float:ap = TF2Attrib_GetValue(armorpiercing);
		damage *= ap;
	}
	return Plugin_Changed;
} 
...
public Action:BuildingRegeneration(Handle:timer, any:entity) 
{
	...
	if(BuildingMaxHealth != BuildingHealth)
	{
		...
		if(BuildingRegen != Address_Null)
		{
			...
			new Regeneration = RoundToNearest((buildingHPRegen*BuildingMaxHealth)/100.0);
			if((Regeneration + BuildingHealth) > BuildingMaxHealth)
			{
				new HealthLeft = BuildingMaxHealth - BuildingHealth;
				AddEntHealth(entity,HealthLeft);
			}
			else
			{
	...
	if (StrEqual(SentryObject, "obj_sentrygun"))
	{
		...
		{
			new Float:AmmoRegeneration = TF2Attrib_GetValue(AmmoRegen);
			if(sentryLevel > 1)
			{
