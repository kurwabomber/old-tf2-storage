		...
		else
		{
			...
			if (slot < 3)
			{
				...
				else if (current_class[client] == _:TFClass_Soldier)
				{
					...
					{
						new currentcash = GetEntProp(client, Prop_Send, "m_nCurrency", currentcash);
						SetEntProp(client, Prop_Send, "m_nCurrency",(currentcash-(client_spent_money[client][0])));
					}				
					else
					{
						currentitem_catidx[client][slot] = GetUpgrade_CatList(classname)
...
public Event_PlayerreSpawn(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	TF2_AddCondition(client, TFCond_UberchargedHidden, 1.8);
	current_class[client] = _:TF2_GetPlayerClass(client)
//	new team = GetClientOfUserId(GetEventInt(event, "team"));
	...
	{
		CreateTimer(0.35, WeaponReGiveUpgrades, GetClientUserId(client));
	}
	if(IsFakeClient(client) || MoneyBonusKill == 5000)
	{
		...
		{
			TF2Attrib_SetByName(client,"dmg taken increased",(15000.0/(additionalstartmoney+StartMoney)));//Damage Taken Increased(decreased)
		}
		else
		{
			TF2Attrib_SetByName(client,"dmg taken increased",(10000.0/(additionalstartmoney+StartMoney)));//Damage Taken Increased(decreased)
		}
		if((additionalstartmoney+StartMoney) <= 1000000){
		TF2Attrib_SetByName(client,"damage penalty",((additionalstartmoney+StartMoney)/19500.0));//Damage Bonus
		TF2Attrib_SetByName(client,"dmg penalty vs players",((additionalstartmoney+StartMoney)/26500.0));//Damage penalty vs players(ap)
		}else{
		TF2Attrib_SetByName(client,"damage penalty",((additionalstartmoney+StartMoney)/23000.0));//Damage Bonus
		TF2Attrib_SetByName(client,"dmg penalty vs players",((additionalstartmoney+StartMoney)/29000.0));//Damage penalty vs players(ap)		
		}
		...
		{
			TF2Attrib_SetByName(client,"max health additive bonus",((additionalstartmoney+StartMoney)/230.0));//Health Bonus
		...
		{
			TF2Attrib_SetByName(client,"max health additive bonus",((additionalstartmoney+StartMoney)/150.0));//Health Bonus
		...
		{
			TF2Attrib_SetByName(client,"max health additive bonus",((additionalstartmoney+StartMoney)/175.0));//Health Bonus
			TF2Attrib_SetByName(client,"clip size bonus",((additionalstartmoney+StartMoney)/12500.0));//Clip Size
		...
		//	TF2Attrib_SetByName(client,"clip size bonus",((additionalstartmoney+StartMoney)/15000.0));//Clip Size
		//	TF2Attrib_SetByName(client,"health regen",((additionalstartmoney+StartMoney)/1000.0));//Health Regen
		//	TF2Attrib_SetByName(client,"engy sentry damage bonus",((additionalstartmoney+StartMoney)/10000.0));//Sentry Damage
		//	TF2Attrib_SetByName(client,"engy sentry fire rate increased",(15000.0/(additionalstartmoney+StartMoney)));//sentry firerate
		//	TF2Attrib_SetByName(client,"engy building health bonus",(0.00035*(additionalstartmoney+StartMoney)));//Building Health
		...
		//	}
		//	else if((additionalstartmoney+StartMoney) >= 100000)
		//	{
		//		TF2Attrib_SetByName(client,"engy building health bonus",(0.0025*(additionalstartmoney+StartMoney)));//Building Health
		//		TF2Attrib_SetByName(client,"engy sentry damage bonus",((additionalstartmoney+StartMoney)/6000.0));//Sentry Damage
		//		TF2Attrib_SetByName(pda,"engy sentry damage bonus",((additionalstartmoney+StartMoney)/35000.0));//Sentry Damage
		//	}
		//	else if((additionalstartmoney+StartMoney) >= 500000)
		//	{
		//		TF2Attrib_SetByName(client,"engy building health bonus",(0.005*(additionalstartmoney+StartMoney)));//Building Health
		//		TF2Attrib_SetByName(client,"engy sentry damage bonus",((additionalstartmoney+StartMoney)/5500.0));//Sentry Damage
		//		TF2Attrib_SetByName(pda,"engy sentry damage bonus",((additionalstartmoney+StartMoney)/30000.0));//Sentry Damage
		//	}
		//	if((additionalstartmoney+StartMoney) <= 75000){
		//	TF2Attrib_SetByName(client,"bullets per shot bonus",((additionalstartmoney+StartMoney)/15000.0));//Bullets Per shot
		///	}else{
		...
		else if (current_class[client] == _:TFClass_Heavy)
		{
			TF2Attrib_SetByName(client,"max health additive bonus",((additionalstartmoney+StartMoney)/200.0));//Health Bonus
			TF2Attrib_SetByName(client,"health regen",((additionalstartmoney+StartMoney)/1000.0));//Health Regen
			TF2Attrib_SetByName(client,"dmg taken increased",(11500.0/(additionalstartmoney+StartMoney)));//Damage Taken Increased(decreased)
		...
		{
			TF2Attrib_SetByName(client,"max health additive bonus",((additionalstartmoney+StartMoney)/225.0));//Health Bonus
		...
		{
			TF2Attrib_SetByName(client,"max health additive bonus",((additionalstartmoney+StartMoney)/400.0));//Health Bonus
...
//		TF2_RespawnPlayer(client);
	...
	else if(attack != client && IsMvM())
	{
		...
		{ 
			...
			{
				new currentcash = GetEntProp(i, Prop_Send, "m_nCurrency", currentcash);
				SetEntProp(i, Prop_Send, "m_nCurrency", ((MoneyBonusKill*mvmadditional) + currentcash));
			} 
		}  
		additionalstartmoney += (MoneyBonusKill*mvmadditional);		
	}
	if(IsFakeClient(client) || MoneyBonusKill == 5000)
	{
		if(!IsMvM()){
...
public UberShopinitMenusHandlers()
{
	...
	HookEvent("npc_hurt",Event_NPCHurt, EventHookMode_Pre);
...
public Action:OnTakeDamage(victim, &attacker, &inflictor, &Float:damage, &damagetype, &weapon, Float:damageForce[3], Float:damagePosition[3], damagecustom)
{
	...
	new Float:Attr1;
	new Address:Attribute1 = TF2Attrib_GetByName(attacker, "absorb damage while cloaked");
	Attr1 = TF2Attrib_GetValue(Attribute1);
...
public Action:Event_NPCHurt(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	damage*=(5000/(StartMoney+additionalstartmoney))
}
...
RespawnEffect(client)
{
	...
	if(current_class[client] == _:TFClass_Pyro)
	{
		TF2Attrib_SetByName(client,"airblast_pushback_no_viewpunch", 1.0);//fix afterburn
		TF2Attrib_SetByName(client,"airblast_pushback_no_stun", 1.0);//fix afterburn
	}
